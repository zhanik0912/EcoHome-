...
// СЫРТТАН setUser ФУНКЦИЯСЫН ҚАБЫЛДАУ КЕРЕК
const Login = ({ setUser }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.ok) {
                // Серверден келген деректі анықтаймыз
                const userData = data.user || data; 
                
                // 1. LocalStorage-ға сақтау
                localStorage.setItem('user', JSON.stringify(userData));
                
                // 2. App.js-тегі state-ті бірден жаңарту (Аватар шығуы үшін)
                if (setUser) {
                    setUser(userData);
                }
                ...
                toast.error(data.error || "Email немесе құпия сөз қате");
            }
        } catch (error) {
